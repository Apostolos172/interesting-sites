...
// εμφάνιση διαφόρων elements με php
	...
	function opendivwithid($id='', $class='')
	{
		if($id!=null)
			//print "<div id=$id>";
			print "<div id=$id class=$class>";
		else
			print "<div>";
	}
	...
	function openFieldset($class='')
	{
		if($class!=null)
			print "<fieldset class=$class>";
		else
			print "<fieldset>";
	}
	...
	function addClassHavingId($id, $class)
	{
		echo "
		<script>
	...
	function changeColorHavingId($id,$color)
	{
		js("
		var element = document.getElementById('$id');
	...
	//Η συνάρτηση αυτή ανοίγει την ομάδα
	function openATeamFrame($legend, $id)
	{
		opendivwithid("$id", "group_div");
			openFieldset('group');
				legend("$legend");
	}
	
	function closeATeamFrame()
	{
			closeFieldset();
			n();
		closediv();
	...
	function createASiteFrame($url='https://www.taxheaven.gr/', $image='https://www.taxheaven.gr/assets/front/images/logo.png')
	{
		opendiv("site_div");
		n();
			echo "<a href='$url'>";
